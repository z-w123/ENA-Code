...
#logs in to correct dir on ebi-cli + activate aws + creates UUID dir on ebi-cli & then copy files from covid-utils upload area to ebi-cli location 
...
#not sure yet how to automate user input for aws?
#if we add more commands to this script, will they run in the ssh terminal or a normal one? No commands will not run, expects user typed input only

#part 2: copies files from ebi-cli dir back to local machine

mkdir submission_files_${UUID} #creates drag n drop submission directory on local machine (which files incl. spreadsheet will be transferred to)
...
#	option 2 : write an expect script cont. credentials and then call that from this bash script (but this will probably only work on ubuntu- and is tricky)